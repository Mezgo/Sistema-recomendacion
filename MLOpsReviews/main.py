...
@app.get('/get_map_name_duration')
def get_map_name_duration(year:int | None = None, platform:str | None = None, duration_type:str | None = None):
    '''Gets the map_nameimum movie duration of the dataset.'''
    temp_df = sp_dataset
    ...
    return f'The longest show is <{temp_df["title"].iloc[0]}>. It belongs to {p_name} & was released in {temp_df["release_year"].iloc[0]}. The duration is {(temp_df["duration_int"].iloc[0])} {temp_df["duration_type"].iloc[0]}'
...
def get_score_count(scored:float, year:str, platform:str):
    ...
    qant = temp_df.shape
    # platform name
    p_name = platform_dicc.get(temp_df.indep_name[0][0])
    return f'On {p_name}, {qant[0]} is the amount of shows that are upon {scored} on the {year}'
...
def get_count_platform(platform:str):
    ...
    p_name = platform_dicc.get(temp_df['id'].iloc[0][0])
    return f'The total amount of shows in {p_name} is: {temp_df["id"].count()}'
